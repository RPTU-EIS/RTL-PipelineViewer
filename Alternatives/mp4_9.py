...
for key, signal_name in ex_signals.items():
    # --- THIS IS THE FIX: Changed base from 10 to 2 ---
...
for cycle_idx in range(num_cycles):
    ...
    hazard_source_pcs = set()
    
    # --- Enhanced Hazard Detection ---
    id_pc = cycle_stage_pc[cycle_idx].get("ID")
    ex_pc = cycle_stage_pc[cycle_idx].get("EX")
    mem_pc = cycle_stage_pc[cycle_idx].get("MEM")
    wb_pc = cycle_stage_pc[cycle_idx].get("WB")

    if id_pc is not None:
        rs1 = delayed_hazard_data_by_cycle[cycle_idx].get("rs1_addr")
        rs2 = delayed_hazard_data_by_cycle[cycle_idx].get("rs2_addr")

        # RAW Hazard from EX stage
        if ex_pc is not None and forwardA_val == 1:
            hazards_for_js.append({
                "cycle": cycle_idx,
                "type": "RAW",
                "producer_pc": ex_pc,
                "consumer_pc": id_pc,
            ...
            hazard_source_pcs.add(ex_pc)
        if ex_pc is not None and forwardB_val == 1:
            hazards_for_js.append({
                "cycle": cycle_idx,
                "type": "RAW",
                "producer_pc": ex_pc,
        ...
        # RAW Hazard from MEM stage
        if mem_pc is not None and forwardA_val == 2:
            hazards_for_js.append({
                "cycle": cycle_idx,
                "type": "RAW",
                "producer_pc": mem_pc,
            ...
            hazard_source_pcs.add(mem_pc)
        if mem_pc is not None and forwardB_val == 2:
            hazards_for_js.append({
                "cycle": cycle_idx,
                "type": "RAW",
                "producer_pc": mem_pc,
    ...
    for stage, actual_pc in cycle_stage_pc[cycle_idx].items():
        if actual_pc is not None and actual_pc in vcd_actual_to_synthetic_pc_map:
            ...
            display_text = stage # Default display text is just the stage name
            hazard_info = {"forwardA": 0, "forwardB": 0}
            
            is_hazard_source = actual_pc in hazard_source_pcs
            if is_hazard_source:
                tooltip_content += "\n--- Hazard Source ---"
            ...
            if stage == "EX":
                ...
                # --- NEW: Add ALU operation details to display text and tooltip ---
                ...
                # If there is no valid decoded instruction or missing operands/results
                if (not disassembled_text) or str(disassembled_text).startswith("N/A_ASM") \
                or (op_a is None or op_b is None or result is None):
                    display_text = "No instruction fetched"
                    tooltip_content += "\nNo instruction fetched"
                else:
                    # Map ALU mnemonics to symbols (plain for tooltip)
                    operator_map_plain = {
                        "ADDI": "+", "ADD": "+", "SUB": "-",
                        "AND": "&", "ANDI": "&",
                        "OR":  "|", "ORI":  "|",
                    ...
                    # Get mnemonic and pick operator
                    mnemonic = disassembled_text.split()[0].upper() if ' ' in disassembled_text else disassembled_text.upper()
                    operator_plain = operator_map_plain.get(mnemonic, mnemonic)

                    # Escape < for HTML in the EX cell so it renders correctly
                    operator_html = operator_plain.replace("<", "&lt;")

                    # Compact equation for EX cell
                    display_text = f"EX<br>{op_a} {operator_html} {op_b} = {result}"

                    # Tooltip keeps plain version
                    tooltip_content += f"\n--- ALU Op ---\n{op_a} {operator_plain} {op_b} = {result}"
...
<script>
    ...
    const hazardData = {hazards_for_js};
...
""".format(
    ...
    hazards_for_js=json.dumps(hazards_for_js),
