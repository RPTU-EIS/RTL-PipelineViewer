...
4. Pipeline activity from the VCD is mapped **in program-order-of-first-appearance** onto these re-indexed PCs. That means: the *first unique actual PC value observed* in the VCD is treated as *instruction 0*, the second unique actual PC observed as *instruction 1*, etc., and we LABEL those rows using the re-indexed PC addresses (0x00000000, 0x00000004, ...) so that your HTML shows exactly the format you requested: `PC_0xXXXXXXXX | XXXXXXXX` (where XXXXXXXX is the instruction hex from VCD).
...
Outputs:
  ...
  - pipeline_matrix.html         (color, tooltips, search; rows show Re-indexed PC | HEX from VCD)
...
output_csv = "pipeline_matrix_reindexed_pcs.csv" # Updated output filename
output_html = "pipeline_matrix_reindexed_pcs.html" # Updated output filename
    ...
    for t in all_signal_times_for_stage:
        while pc_idx < len(pc_tv) and pc_tv[pc_idx][0] <= t:
            try:
                ...
                current_pc_val = None # Keep as None if 'x' or invalid
            pc_idx += 1
...
for stage, signal_name in stage_signals.items():
    ...
    # Find the first valid PC value to initialize last_valid_pc_seen for this signal
    ...
    for _, val in signal_tv:
        if 'x' not in val.lower():
            try:
                ...
                pass # Continue searching for a valid initial value
    
    tv_idx = 0 # Pointer for signal_tv for current stage
    for cycle_idx, rise_time in enumerate(rising_edges):
        # Advance tv_idx to find the latest valid value at or before 'rise_time'
        # This loop updates `last_valid_pc_seen` to the state of the signal at `rise_time`.
        while tv_idx < len(signal_tv) and signal_tv[tv_idx][0] <= rise_time:
            val = signal_tv[tv_idx][1]
            if 'x' not in val.lower(): # Only update if the value is not 'x'
                try:
                    last_valid_pc_seen = int(val, 16)
                except ValueError:
                    # This should ideally not happen if 'x' is filtered, but for robustness:
                    pass 
            tv_idx += 1
        
        # Assign the last valid PC value seen to the current cycle and stage
...
if initial_actual_pc_for_target_instr is not None:
    ...
    print(f"✅ Explicitly mapped actual PC 0x{initial_actual_pc_for_target_instr:08x} (for instruction 0x{TARGET_INSTR_HEX_BINARY_FOR_SYNTH_0}) to re-indexed PC 0x00000000.")
else:
    print(f"⚠️ Warning: Instruction 0x{TARGET_INSTR_HEX_BINARY_FOR_SYNTH_0} not found in VCD instruction signals. Re-indexed PC 0x00000000 will be assigned to the first observed PC.")
...
print(f"Assigned re-indexed PCs to {len(vcd_unique_actual_pcs_ordered)} unique actual PCs from VCD.")
...
for cycle_idx, stages_in_cycle in enumerate(cycle_stage_pc):
    for stage, actual_pc in stages_in_cycle.items():
        if actual_pc is not None and actual_pc in vcd_actual_to_synthetic_pc_map:
            ...
            # This 'if' block is mostly for safety; rows should be initialized already.
            ...
            # Logic for filling the matrix cell (and detecting stalls)
            current_cell_value = instruction_matrix[display_label][cycle_idx]
            if current_cell_value == "": # Only fill if the cell is currently empty
            ...
            tooltips_matrix[display_label][cycle_idx] = (
                ...
                f"PC (Re-indexed): 0x{synthetic_pc:08x}\n"
...
# This re-run of stall detection helps if an instruction was initially marked with a stage,
# but a subsequent stall condition should override it.
for row_idx, row in df.iterrows():
    for i in range(1, len(row)):
        current_cell_value = row.iloc[i]
        previous_cell_value = row.iloc[i-1]
        
        # A stall occurs if the instruction remains in the same "processing" stage (ID, EX, MEM)
        # from the previous cycle. WB and IF are typically not "stalled" in the same way.
        if current_cell_value == previous_cell_value and current_cell_value in ['ID', 'EX', 'MEM']:
            df.at[row_idx, df.columns[i]] = 'STALL'
            
            # Update tooltip to reflect the stall
...
<body>
<h2>Pipeline Visualization (Re-indexed PCs)</h2>
