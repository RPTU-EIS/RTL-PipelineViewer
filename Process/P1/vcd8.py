# use of the traking pc 
#  use pc in PC_0x00000100
...
if os.path.exists(BINARY_FILE):
    with open(BINARY_FILE, 'r') as f:
        for i, line in enumerate(f):
            ...
            if instr:
                pc = i * 4
                instr_bytes = bytes.fromhex(instr.zfill(8))
                asm = instr  # fallback
...
for pc, asm in pc_to_instr.items():
    print(f"PC_0x{pc:08x}: {asm}") # MODIFIED LINE: Added "PC_" prefix
...
for stage, signal_name in stage_signals.items():
    ...
    # Sort time-value pairs for the current signal chronologically
    sorted_signal_tv = sorted(signal_tv, key=lambda x: x[0])

    # Initialize last_pc with the first valid PC value found for this signal
    # This ensures that if a PC is already present at time 0 or before the first clock edge, it's considered.
    last_pc = None
    for _, val in sorted_signal_tv:
        try:
            initial_pc_val = int(val, 16)
            last_pc = initial_pc_val
            break # Found the first valid PC, stop looking
        except ValueError:
            continue # Skip invalid values like 'x'

    idx = 0 # Current cycle index

    # Iterate through all changes for the signal
    for t, val in sorted_signal_tv:
        try:
            current_signal_pc_val = int(val, 16)
            last_pc = current_signal_pc_val # Update last_pc with the latest valid PC
        except ValueError:
            # If the value is invalid (e.g., 'x'), keep the last valid PC if any, or None
            pass

        # Advance through cycles as long as the current cycle's rising edge is <= current signal time 't'
        while idx < num_cycles and rising_edges[idx] <= t:
            if last_pc is not None:
                cycle_stage_pc[idx][stage] = last_pc
            idx += 1

    # After processing all time-value changes for a signal,
    # if last_pc is still valid and there are remaining cycles, fill them.
    # This handles cases where a PC value remains stable for many subsequent cycles.
...
for cycle_idx, stages in enumerate(cycle_stage_pc):
    for stage, pc in stages.items():
        if pc is not None:
            ...
            display_label = f"{label} | {instr_text}" if instr_text else label
...
# The index of the DataFrame (before sorting) represents the unique PCs found in VCD signals
for pc_label in df.index:
    # pc_label will be like "PC_0x00000100 | li ra,0"
    print(pc_label.split(' | ')[0]) # Print just the PC_0x... part
print("--------------------------------------------------\n")
