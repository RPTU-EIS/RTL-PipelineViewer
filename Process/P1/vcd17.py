...
for stage_name, pc_signal_name in stage_signals.items():
    ...
    for t in all_signal_times_for_stage:
        ...
        if current_pc_val is not None and current_instr_raw_val is not None and 'x' not in current_instr_raw_val.lower():
            ...
            if current_pc_val not in actual_pc_to_disassembled_instr: # Disassemble only once per unique PC
                disassembled_text = "N/A_ASM"
                if instr_hex_for_bytes:
                    try:
                        instr_bytes = bytes.fromhex(instr_hex_for_bytes)
                        # !!! Crucial Change: Reverse bytes for little-endian interpretation by Capstone
                        ...
                        if disassembled:
                            ...
                            disassembled_text = "N/A_ASM (Empty Disassembly)" # More descriptive
                    except Exception as e:
                        disassembled_text = f"N/A_ASM (Error: {e})"
                actual_pc_to_disassembled_instr[current_pc_val] = disassembled_text
...
for cycle_idx, stages_in_cycle in enumerate(cycle_stage_pc):
    for stage, actual_pc in stages_in_cycle.items():
        if actual_pc is not None and actual_pc in vcd_actual_to_synthetic_pc_map:
            ...
            # Get pre-calculated hex display and disassembled instruction for tooltips
            ...
            # The display_label must match the row label format
...
# --- Print a summary of PC to Assembly mapping (new function) ---
print("\n--- Summary of Re-indexed PC, Actual PC, Hex, and Disassembled Instruction ---")
...
for actual_pc, synthetic_pc in sorted_pcs_for_print:
    instr_hex_display = actual_pc_to_instr_hex_display.get(actual_pc, "N/A_HEX")
    disassembled = actual_pc_to_disassembled_instr.get(actual_pc, "N/A_ASM")
    print(f"Re-indexed PC: 0x{synthetic_pc:08x} | Actual PC: 0x{actual_pc:08x} | Hex: {instr_hex_display} | ASM: {disassembled}")
print("-----------------------------------------------------------------------------------\n")
...
for idx in df.index:
    ...
    for c_idx, val in enumerate(df.loc[idx]):
        ...
        cell_content = val # Default content for the table cell
        
        # --- NEW LOGIC FOR TOOLTIP DISPLAY ---
        # Only add tooltip HTML structure if the stage is 'ID' and there's valid tooltip text
        if val == 'ID' and tooltips.loc[idx][c_idx]:
            tooltip_text = tooltips.loc[idx][c_idx]
            cell_content = f'<div class="tooltip">{val}<span class="tooltiptext">{tooltip_text}</span></div>'
        # --- END NEW LOGIC ---
