...
# from vcd.writer import VCDWriter # REMOVED: Not needed if VCD output is removed
import pandas as pd
...
# output_vcd = "pipeline_cycle_trace.vcd" # REMOVED: No VCD output
...
# This dictionary will store actual VCD PCs mapped to their observed instruction hex (binary string)
...
# --- FIXED: PC and instruction are matched strictly by same timestamp ---
...
for stage_name, pc_signal_name in stage_signals.items():
    ...
    # Build quick-access dictionaries for time -> value
    pc_by_time = {}
    for t, v in pc_tv:
        try:
            pc_by_time[t] = int(v, 16)
        except ValueError:
            continue
    ...
    for t, v in instr_tv:
        if 'x' not in v.lower():
            clean_bin = ''.join(c for c in v if c in '01')[-32:].zfill(32)
            instr_by_time[t] = clean_bin

    # Map only when both signals update at the same time
    common_times = sorted(set(pc_by_time.keys()) & set(instr_by_time.keys()))
    for t in common_times:
        pc_val = pc_by_time[t]
        instr_val = instr_by_time[t]
        if pc_val not in actual_pc_to_instr_hex_binary:
            actual_pc_to_instr_hex_binary[pc_val] = instr_val

print(f"📋 Matched {len(actual_pc_to_instr_hex_binary)} PC → Instruction pairs from VCD (same cycle+stage).")
...
for actual_pc in vcd_unique_actual_pcs_ordered:
    ...
    # Get the instruction hex (binary string) from the map populated directly from VCD instructions
    instr_hex_binary = actual_pc_to_instr_hex_binary.get(actual_pc, "N/A_INSTR")
    
    # Convert binary string to 8-digit hexadecimal string for display
    if instr_hex_binary != "N/A_INSTR" and all(c in '01' for c in instr_hex_binary):
        instr_hex_display = f"{int(instr_hex_binary, 2):08x}"
    else:
        instr_hex_display = instr_hex_binary # Keep as is if not a valid binary string (e.g., 'x')
    
    row_label = f"PC_0x{synthetic_pc:08x} | {instr_hex_display}" # Use the formatted hex
    ordered_row_labels.append(row_label)

# --- REMOVED: Manual override for PC_0x00000000 (synthetic) ---
# The presence and stage of PC_0x00000000 will now solely depend on the VCD trace.
# The `ordered_row_labels` already ensures all observed PCs from VCD are included.
...
for cycle_idx, stages_in_cycle in enumerate(cycle_stage_pc):
    for stage, actual_pc in stages_in_cycle.items():
        if actual_pc is not None and actual_pc in vcd_actual_to_synthetic_pc_map:
            synthetic_pc = vcd_actual_to_synthetic_pc_map[actual_pc]
            instr_hex_binary = actual_pc_to_instr_hex_binary.get(actual_pc, "N/A_INSTR") # Get original binary string
            ...
            elif instruction_matrix[display_label][cycle_idx] == "": # Only fill if empty
                instruction_matrix[display_label][cycle_idx] = stage
            
            # Generate tooltip text
            disassembled_instr = instr_hex_binary # Fallback, use the binary string
            try:
                if instr_hex_binary != "N/A_INSTR" and all(c in '01' for c in instr_hex_binary):
                    # Convert binary string to hex string for bytes.fromhex
            ...
            except ValueError: # e.g., non-binary characters or invalid hex after conversion
    ...
    for i in range(1, len(row)):
        ...
        if current_cell_value == previous_cell_value and current_cell_value in ['ID', 'EX', 'MEM']:
            ...
            if "(STALL)" not in current_tooltip: # Avoid duplicate "(STALL)"
                # Replace the old stage name in the tooltip with the new STALL status
                # This assumes the tooltip always has "Stage: [STAGE_NAME]"
...
#                     signal_name_in_vcd = f"synth_PC_{clean_synth_pc_hex}_{stage}"
#                     if len(signal_name_in_vcd) > 250:
...
#         writer.change(0, None, None) # Ensure time 0 is written to mark initial state

#         if signal_refs:
#             for sig_var_obj in signal_refs.values():
#                 writer.change(0, sig_var_obj, '0') 

#         for c_idx, col_name in enumerate(df.columns):
#             current_time_ns = rising_edges[c_idx] 
#             writer.change(current_time_ns, None, None)
...
#                         is_active = '1' if cell_value == stage or cell_value == 'STALL' else '0'
...
print("\n📋 Instructions fetched from VCD by stage:")

for stage, inst_sig in instruction_signals.items():
    if inst_sig not in vcd.signals:
        print(f"❌ {stage}: Signal '{inst_sig}' not found in VCD")
        continue

    inst_values = set()
    for t, val in vcd[inst_sig].tv:
        # normalize and clean value
        val = val.lower().replace('x', '0')
        val = ''.join(c for c in val if c in '0123456789abcdef')[-8:]
        if val:
            inst_values.add(val.zfill(8))

    sorted_list = sorted(inst_values)
    print(f"{stage}: {sorted_list}")


print("\n🔎 DEBUG: Full PC → Instruction Map")
for pc, instr_bin in actual_pc_to_instr_hex_binary.items():
    print(f"PC_0x{pc:08x} → {int(instr_bin, 2):08x}")
