...
for reg_name, signal_name in register_signals.items():
    # Register values are usually in hex, so use base 16. Default to 'x' for unknown.
...
for cycle in cycle_stage_pc:
    for stage in stage_signals:
        actual_pc = cycle[stage]
        if actual_pc is not None and actual_pc not in vcd_actual_to_synthetic_pc_map:
...
for synthetic_pc in sorted(pipeline_data_for_js.keys()):
    ...
    instruction_labels_for_js.append(f"0x{synthetic_pc:04x}: {asm_display}")
...
.pipeline-grid {{ grid-template-columns: 350px repeat(5, 100px); }}
.register-grid {{ grid-template-columns: repeat(2, 120px); }}
...
.instruction-label {{ text-align: left; font-weight: bold; font-family: monospace; }}
...
.register-name-cell {{ font-family: monospace; text-align: right !important; padding-right: 15px !important; }}
...
<script>
    ...
    function updatePipelineDisplay() {{
        ...
        instructionLabels.forEach((label, instrIdx) => {{
            const syntheticPc = Object.keys(pipelineData)[instrIdx];
    ...
    function updateRegisterTable() {{
        ...
        for (let i = 0; i < 32; i++) {{
            const regName = `x${{i}}`;
            const regValue = currentRegisterValues ? (currentRegisterValues[regName] || 'N/A') : 'N/A';
            ...
            valueCell.textContent = regValue;
...
""".format(
    ...
    register_data_js=json.dumps(register_values_by_cycle),
