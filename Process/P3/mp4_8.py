...
<div class="content-wrapper">
    <div class="pipeline-container">
        ...
        <div id="pipelineDisplay" class="pipeline-grid">
            <div class="grid-header">PC | Instruction (ASM)</div>
...
<script>
    ...
    function updatePipelineDisplay() {{
        ...
        instructionLabels.forEach((label, instrIdx) => {{
            const syntheticPcStr = Object.keys(vcd_actual_to_synthetic_pc_map).find(key => vcd_actual_to_synthetic_pc_map[key] === (instrIdx * 4));
            if (!syntheticPcStr) return; // Should not happen if data is consistent

            const syntheticPc = vcd_actual_to_synthetic_pc_map[syntheticPcStr];
            ...
            ["IF", "ID", "EX", "MEM", "WB"].forEach(stageName => {{
                ...
                const cycleDataForPc = pipelineData[syntheticPc];
    ...
    function updateRegisterTable() {{
        ...
        for (let i = 0; i < 32; i++) {{
            const regName = `x${{i}} (${{abiNames[i]}})`;
            const regValue = currentRegisterValues ? (currentRegisterValues[`x${{i}}`] || 'N/A') : 'N/A';
