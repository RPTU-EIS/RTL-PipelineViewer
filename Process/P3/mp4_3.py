...
hazard_signals = {
    ...
    "rs1_addr": "HazardDetectionRV32I.core.IDBarrier.io_outRS1", # Assuming ID stage outputs rs1/rs2 addresses
    "rs2_addr": "HazardDetectionRV32I.core.IDBarrier.io_outRS2",

    # Destination register addresses from MEM and WB stages
    "rd_mem_addr": "HazardDetectionRV32I.core.EXBarrier.io_outRD", # Assuming this is the RD from MEM/WB boundary
    "rd_wb_addr": "HazardDetectionRV32I.core.MEMBarrier.io_outRD", # Assuming this is the RD from WB stage
...
hazard_data_by_cycle = [{} for _ in range(num_cycles)] # NEW: To store hazard signals
...
for key, signal_name in hazard_signals.items():
    ...
    for cycle_idx, val in enumerate(vals):
        hazard_data_by_cycle[cycle_idx][key] = val
...
# NEW: Calculate RD_wb_2 (2-cycle delayed WB RD)
    ...
    for stage, actual_pc in cycle_stage_pc[cycle_idx].items():
        if actual_pc is not None and actual_pc in vcd_actual_to_synthetic_pc_map:
            ...
            if stage == "EX":
                ...
                # NEW: Add hazard detection info for EX stage
                current_hazard_data = hazard_data_by_cycle[cycle_idx]
...
.forwarding-highlight {{
    border: 2px solid #ff0000; /* Red border for forwarding */
