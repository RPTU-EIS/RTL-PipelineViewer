        ...
        if actual_pc is not None and actual_pc in vcd_actual_to_synthetic_pc_map:
            ...
            # --- FIX FOR EX STAGE TOOLTIP ---
                ...
                if op_a is not None and op_b is not None and result is not None:
                    ...
                    tooltip += f"{op_a} {operator} {op_b}\n" # Added "ALU Operation:" for clarity
                    tooltip += f"Result: {result}" # Added "ALU Result:" for clarity
                else:
...
for idx in df.index:
    ...
    for c_idx, val in enumerate(df.loc[idx]):
        ...
        # Show stage and brief info in the cell for IF, ID, EX stages
        if val in ["IF", "ID", "EX"]:
            display_info = ""
            
            if val in ["IF", "ID"] and asm_line:
                display_info = asm_line
            
            elif val == "EX" and pd.notna(tooltip_text):
                # Extract EX info: A OP B = RESULT
                alu_line = ""
                result_line = ""
                for line in tooltip_text.split("\n"):
                    if re.match(r"^\d+ [A-Z]+ \d+", line):  # Matches lines like "4 ADD 5"
                        alu_line = line.strip()
                    elif line.startswith("Result:"):
                        result_line = line.strip().split("Result:")[-1].strip()

                if alu_line and result_line:
                    display_info = f"{alu_line} = {result_line}"  # e.g., "4 ADD 5 = 9"

            if display_info:
                cell_content = (
                    f'<div class="tooltip">{val}<br>'
                    f'<span style="font-size:9px; color:#000;">{display_info}</span>'
