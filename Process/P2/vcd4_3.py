...
pc_values_by_cycle = [None] * num_cycles # Added to store PC for each cycle
...
pc_signal_name = stage_signals["EX"]
if pc_signal_name in vcd.signals:
    pc_tv_sorted = sorted(vcd[pc_signal_name].tv, key=lambda x: x[0])
    last_pc_val = None
    pc_idx = 0
    for cycle_idx, rise_time in enumerate(rising_edges):
        while pc_idx < len(pc_tv_sorted) and pc_tv_sorted[pc_idx][0] <= rise_time:
            ...
            if 'x' not in val.lower():
                try:
                    ...
                    pass
            pc_idx += 1
        pc_values_by_cycle[cycle_idx] = last_pc_val
...
for i in range(min(15, len(ex_values_by_cycle))):
    ...
    pc = pc_values_by_cycle[i]
    if all(k in entry for k in ["operandA", "operandB", "aluResult"]) and pc is not None:
