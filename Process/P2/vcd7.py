...
# --- Prepare delayed WB stage data for tooltips ---
# The WB stage data (rd, wb_data) from cycle N-1 are relevant for the instruction
# that is in the WB stage during cycle N.
    ...
    for stage, actual_pc in stages_in_cycle.items():
        if actual_pc is not None and actual_pc in vcd_actual_to_synthetic_pc_map:
            ...
            # --- NEW WB STAGE TOOLTIP ---
                ...
                # Use the delayed WB data
                wb_data = delayed_wb_values_by_cycle[cycle_idx] if cycle_idx < len(delayed_wb_values_by_cycle) and delayed_wb_values_by_cycle[cycle_idx] is not None else {}
                rd_val = wb_data.get("rd")
                wb_data_val = wb_data.get("wb_data") # Assuming a signal for data written back

                tooltip = f"Stage: {stage}\n"
                # tooltip += f"PC: 0x{actual_pc:08x}\n"
                tooltip += f"ASM: {disassembled_instr_for_tooltip}\n"
                if rd_val is not None:
                    tooltip += f"Write Register \n"
                    tooltip += f"rd (${rd_val}) = {wb_data_val}\n"
        ...
        if pd.notna(tooltip_text):
            for line in tooltip_text.split("\n"):
                if line.startswith("Instruction (ASM):"):
                    ...
                    # Match lines like: rd ($5) = 0x0000000f
                    match = re.match(r"rd \(\$(\d+)\) = (.+)", line)
                    if match:
                        wb_rd_line = f"Rd: {match.group(1)}"
                        wb_data_line = f"Data: {match.group(2)}"
        
        # Show stage and brief info in the cell for IF, ID, EX, WB stages
        if val in ["IF", "ID","WB"] and asm_line:
            cell_content = (
...
for i in range(min(15, len(delayed_wb_values_by_cycle))): # Use delayed_wb_values_by_cycle here
    ...
    if entry and all(k in entry for k in ["rd", "wb_data"]) and pc is not None:
        print(f"Cycle {i} (PC: 0x{pc:08x}): Write Register {entry['rd']} <= {entry['rd_val']}")
    else:
        print(f"Cycle {i}: Incomplete data (or no previous cycle data for cycle 0)")
print("----------------------------------------------------------")
