    ...
    for stage, actual_pc in cycle_stage_pc[cycle_idx].items():
        if actual_pc is not None and actual_pc in vcd_actual_to_synthetic_pc_map:
            ...
            if stage == "EX":
                ...
                # Map ALU mnemonics to math symbols
                operator_map = {
                    "ADDI": "+",
                    "ADD":  "+",
                    "SUB":  "-",
                    ...
                    "SLT":  "<",    # Set Less Than
                    "SLTI": "<",
                    "SLTU": "<u",   # Unsigned comparison (optional: "<" if you don't want the 'u')
                    "SLTIU": "<u"
                }

                # Extract the mnemonic from the disassembly
                mnemonic = disassembled_text.split()[0].upper() if ' ' in disassembled_text else disassembled_text.upper()

                # Use symbol if we know it, otherwise fall back to the mnemonic
                operator = operator_map.get(mnemonic, mnemonic)


                if op_a is not None and op_b is not None and result is not None:
                    # Format for multi-line display inside the EX box
                    display_text = f"EX<br>{op_a} {operator} {op_b} = {result}"
                    # Add to tooltip as well
                    tooltip_content += f"\n--- ALU Op ---\n{op_a} {operator} {op_b} = {result}"
