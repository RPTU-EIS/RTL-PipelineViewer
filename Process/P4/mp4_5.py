...
for key, signal_name in ex_signals.items():
    vals = extract_signal_at_cycles(signal_name, default_val=None, base=10) # Using base 10 for ALU values
    ...
    for stage, actual_pc in cycle_stage_pc[cycle_idx].items():
        if actual_pc is not None and actual_pc in vcd_actual_to_synthetic_pc_map:
            ...
            if stage == "EX":
                ...
                operator = disassembled_text.split()[0].upper() if ' ' in disassembled_text else disassembled_text.upper()

                if op_a is not None and op_b is not None and result is not None:
                    # Format for multi-line display inside the EX box
                    display_text = f"EX<br>Op: {op_a} {operator} {op_b}<br>Res: {result}"
        ...
        instructionLabels.forEach((label, instrIdx) => {{
            ...
            ["IF", "ID", "EX", "MEM", "WB"].forEach(stageName => {{
                ...
                if (instrCycleData && instrCycleData.stage === stageName) {{
                    ...
                    // --- MODIFICATION: Use innerHTML to render multi-line content from Python ---
