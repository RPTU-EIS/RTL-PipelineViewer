...
for cycle_idx in range(num_cycles):
    # First, handle the hazard sources based on forwarding signals
    ...
    # Identify the PC of the instruction that is the source of the hazard
    if forwardA_val == 1 or forwardB_val == 1:
        # Source is from EX/MEM, which is the instruction in the MEM stage this cycle
        source_pc = cycle_stage_pc[cycle_idx].get("MEM")
        if source_pc is not None:
            hazard_source_pcs.add(source_pc)
    if forwardA_val == 2 or forwardB_val == 2:
        # Source is from MEM/WB, which is the instruction in the WB stage this cycle
        source_pc = cycle_stage_pc[cycle_idx].get("WB")
        if source_pc is not None:
            hazard_source_pcs.add(source_pc)
    
    # NEW: Print statement for debugging
    if hazard_source_pcs:
        print(f"Cycle {cycle_idx}: Hazard detected. Source PC(s): {[f'0x{pc:08x}' for pc in hazard_source_pcs]}")
    ...
    for stage, actual_pc in cycle_stage_pc[cycle_idx].items():
        if actual_pc is not None and actual_pc in vcd_actual_to_synthetic_pc_map:
            ...
            is_hazard_source = False
            if actual_pc in hazard_source_pcs:
                is_hazard_source = True
                tooltip_content += "\n--- Hazard Source ---"
...
.legend {{ margin: 20px auto; display: flex; justify-content: center; gap: 20px; padding: 10px; border: 1px solid #ddd; border-radius: 8px; width: fit-content; }}
...
/* --- NEW: Highlighting configuration for clarity --- */
/* Style for the instruction PROVIDING the forwarded data */
.hazard-source-highlight {{
    border: 3px solid #FF4500 !important; /* OrangeRed border for the source */
    box-shadow: 0 0 10px rgba(255, 69, 0, 0.7);
}}

/* Style for the EX stage RECEIVING the forwarded data */
.forwarding-destination-highlight {{
    border: 3px solid #007BFF !important; /* Blue border for the destination */
...
<script>
    ...
    function updatePipelineDisplay() {{
        ...
        instructionLabels.forEach((label, instrIdx) => {{
            ...
            // If the instruction in its current stage is a hazard source, highlight the instruction label cell.
            ...
            ["IF", "ID", "EX", "MEM", "WB"].forEach(stageName => {{
                ...
                if (instrCycleData && instrCycleData.stage === stageName) {{
                    ...
                    // Highlight the EX stage if it's the DESTINATION of forwarding
                    ...
                    // Highlight the stage cell if it's the SOURCE of a hazard
    ...
    speedControl.addEventListener('input', () => {{
        ...
        if (animationInterval) {{ // If animation is playing, restart interval with new speed
            stopAnimation();
            startAnimation();
        }}
    }});
